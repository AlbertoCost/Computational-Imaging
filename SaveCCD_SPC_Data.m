...
MANTA = 0;
%% Path
data_folder = 'C:\Users\alby1\OneDrive\Documenti\MATLAB'; 
out_folder = 'C:\Users\alby1\OneDrive\Documenti\MATLAB';
ccd_ext='SPE';
cmos_ext='tif';
tr_ext='sdt'; %if manta -> sdtm

%% Measurement info
day = '20240523';           % (data must be in a folder called as day)
prefix = 'fluo_03';     % filename without extension
N_Pattern_IN = 2048;           % Input pattern number
N_Pattern_OUT = 1;          % Output pattern number

CF = 1; % Select if the SPC has been acquired with Continuous Flow (1 = Yes)

% SPC parmaters
if SPC == 1
    N_lambda = 1; % 16 L16; 1 PMT/SPAD
...
if SPC == 1
    ...
    if CF == 0
        ...
        data = ReadSDT_ContFlow(filepath,Num_chan,N_lambda,(N_Pattern_IN)/(bits/Num_chan),bits/Num_chan);
        spc = squeeze(data);
%         [~,b] = min(sum(data,1:2));
